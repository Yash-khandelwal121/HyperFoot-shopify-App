...
// ---------------------------------------------------------------------------
// LOADER — authenticate + billing check (no changes from before)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ACTION — request billing plan
//
// billing.request() THROWS a redirect Response in @shopify/shopify-app-remix.
// We catch that Response, extract the Location header (the Shopify billing
// approval URL), and return it as JSON to the client.
// The client (useFetcher) then uses useAppBridge to open that URL in the
// Shopify Admin parent frame — this is the correct embedded-app pattern and
// preserves the session on return.
// ---------------------------------------------------------------------------
export const action = async ({ request }) => {
  ...
  try {
    ...
    console.error("Billing request error:", errorOrResponse);
    return {
      error:
        errorOrResponse?.message ||
        "Could not initiate billing. Ensure your app is configured for App Store distribution.",
    };
  }
};
...
export default function Pricing() {
  ...
  // useFetcher keeps the POST within the embedded iframe — no full-page nav.
  const fetcher = useFetcher();
  const navigate = useNavigate();

  // App Bridge gives us the ability to open URLs in the Shopify Admin parent
  // frame, which is the correct way to navigate to the billing approval page
  // while keeping the embedded session intact.
  ...
  // Reset local error when a new submission starts
  useEffect(() => {
  ...
  const isLoading = (plan) =>
    fetcher.state !== "idle" && selectedLoadingPlan === plan.billingName;

  const showError = billingError || localError;
      ...
      {showError && (
        <div style={{
          ...
          <button
            onClick={() => {
              setLocalError(null);
              if (billingError) navigate("/app/pricing", { replace: true });
            }}
            style={{
        ...
        {PLANS_DATA.map((plan) => {
          ...
          return (
            ...
            >
              ...
              {plan.id === "free" ? (
                <button
                  onClick={!disabled ? handleFreeClick : undefined}
                  disabled={disabled}
                  style={{
                    width: "100%",
                    padding: "13px 20px",
                ...
                /*
                 * KEY FIX: Use fetcher.Form instead of native <form>.
                 * fetcher.Form submits via fetch() — no full-page navigation.
                 * The embedded App Bridge session is preserved.
                 * The action returns { confirmationUrl } which we open via
                 * shopify.open() in the useEffect above.
                 */
                <fetcher.Form
                  method="POST"
                  onSubmit={() => setSelectedLoadingPlan(plan.billingName)}
                >
